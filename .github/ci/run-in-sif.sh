...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-stats's —
...
# so only scitex-stats's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_stats-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-stats's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-stats + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-stats's historical bare-uv/pip workflow so a
...
# scitex-stats's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-stats style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-stats-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-stats's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_stats --cov-report=xml --cov-report=term \
    -p no:cacheprovider
